/**
 * Local check: same mTLS client + lease status URL pattern as deploy-step-manifest /
 * sendManifestAndVerify (Deno createHttpClient + fetch).
 *
 ...
 * from `getLeases` + `getProvider` (same as deploy-step-create).
 *
 * Or three PEM files instead of --bundle.
 *
 * Env: AKASH_MANIFEST_NETWORK (default sandbox-2), AKASH_RPC_URL, AKASH_GRPC_URL (same as Edge).
 */
...
} from "../supabase/functions/_shared/akashOrchestrator.ts";
...
async function main(): Promise<void> {
  ...
  if (args.syncUrlFromChain && !mnemonic) {
  ...
  if (!pem) {
    console.error("Could not parse PEM bundle (need --bundle or --cert/--key/--pubkey).\n");
    Deno.exit(1);
  }
  ...
  if (mnemonic) {
    ...
    if (!chain.registered) {
      console.error(
        "\nFAIL: This cert/public key is not in the chain’s certificate list for that owner.\n" +
          "Run deploy-step-cert / check-akash-cert until createCertificate succeeds for the same wallet.",
      );
      Deno.exit(1);
    }

    const { owner: hotOwner, sdk, signer } = await createAkashClients(mnemonic, endpoints).init();
    try {
      ...
      try {
        ...
        console.error(
          "\nThis almost always means this dseq is NOT owned by AKASH_HOT_MNEMONIC’s wallet.\n" +
            "The provider authorizes mTLS using the **deployment tenant’s** registered certs.\n" +
      ...
      if (rows.length === 0) {
        ...
        type LeaseRow = { lease?: { id?: { gseq?: number; oseq?: number; provider?: string }; state?: number } };
        const activeRows = (rows as LeaseRow[]).filter((r) => r.lease?.state === 1);
        for (const r of rows as LeaseRow[]) {
          const id = r.lease?.id;
          if (!id) continue;
          console.log(
            `  lease state=${r.lease?.state} gseq=${id.gseq} oseq=${id.oseq} provider=${id.provider}`,
          );
        }
        ...
        if (activeRows.length > 0 && !wanted && !args.syncUrlFromChain) {
          ...
          console.error(
            `Re-run with --gseq ${first.gseq} --oseq ${first.oseq} or use --sync-url-from-chain.`,
          );
        }
        if (args.syncUrlFromChain) {
          if (activeRows.length === 0) {
            console.error(
              "\nFAIL: --sync-url-from-chain requires an **ACTIVE** on-chain lease (state=1).\n" +
                "Your query returned only non-active leases (e.g. state=3 = closed). The provider\n" +
                "returns 401 for lease/status and manifest when the lease is not active — this is\n" +
                "not an Edge-function bug and not (by itself) proof that the mTLS PEM is wrong.\n" +
                "Create a new deployment + lease, or pick a dseq whose lease is still active.\n",
            );
            Deno.exit(1);
          }
          ...
          console.log(JSON.stringify({ host: probeHost, gseq: probeGseq, oseq: probeOseq, provider: lid.provider }, null, 2));
    ...
    console.log(
      "\n⚠️  AKASH_HOT_MNEMONIC not set — skipping on-chain cert + getDeployment + lease checks.\n" +
        "   Re-run with the same mnemonic as Edge to confirm cert registration and dseq ownership.",
    );
  }
  ...
  console.log("\n--- Provider mTLS (same as deploy-step-manifest lease status) ---");
  ...
  // Optional: manifest PUT with invalid body to see auth vs payload errors (often 401 = auth).
  console.log("\n--- Manifest PUT probe (minimal body; expect 4xx if auth OK) ---");
  console.log("PUT", manifestUrl);
  const putClient = Deno.createHttpClient({ cert: pem.cert, key: pem.privateKey });
  try {
  ...
  if (stStatus === 401 || putStatus === 401) {
    const skippedChain = !Deno.env.get("AKASH_HOT_MNEMONIC")?.trim();
    console.error(
      `\nFAIL: Provider returned HTTP 401 (lease/status=${stStatus}, manifest PUT=${putStatus}).\n` +
        ...
        (skippedChain
          ? "You did **not** set AKASH_HOT_MNEMONIC — the script could not verify on-chain cert registration or that dseq belongs to your wallet.\n" +
            "Export the mnemonic and re-run before assuming the PEM bundle alone is wrong.\n\n"
          : "") +
        "Checklist:\n" +
        ...
        "  5. Re-run with **--sync-url-from-chain** to align host + gseq + oseq with chain (same as deploy-step-create).\n" +
        "  6. AKASH_MANIFEST_NETWORK + RPC/GRPC match the chain (Sandbox-2 defaults use rpc.sandbox-2.aksh.pw unless you override).\n" +
        "  7. If the same URL returns 401 with **curl --cert/--key**, it is not Deno or your Edge function.\n" +
        "  8. If curl with --cert/--key still returns 401, the provider gateway may not accept this cert for that lease; confirm hostUri and lease state on-chain.\n",
    );
    Deno.exit(1);
  }
