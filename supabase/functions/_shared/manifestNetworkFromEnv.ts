...
export function chainSdkManifestNetworkFromEnv(): ChainSdkManifestNetwork {
  const raw = (Deno.env.get("AKASH_MANIFEST_NETWORK") ?? DEFAULT_AKASH_MANIFEST_NETWORK)
    .toLowerCase()
    .trim();
  if (raw === "testnet") return "testnet";
  if (raw === "mainnet") return "mainnet";
  // sandbox-2, sandbox-02, sandbox, etc. → SDK "sandbox" bucket (RPC overrides still apply)
  return "sandbox";
}
